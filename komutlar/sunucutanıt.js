...
exports.run = async (client, message, args) => {
  ...
  .setTitle('BAŞARILI')
  .addField(`Sunucu burada tanıtıldı!`, `(https://discord.gg/G93yznR)`)
  .addField("12 Saatte Bir Sunucu Tanıtımı Yapabilirsiniz!","឵឵")
  .setColor('GREEN')
 message.channel.sendEmbed(embed);
    message.channel.createInvite({maxAge: 0}).then((invite) => {
      ...
      .setThumbnail(message.guild.iconURL)
       client.channels.get('761650226968657960').send(embed)
            });
  ...
  message.channel.send({embed: {
      description: '**Başarısız** \n\nSunucunu Zaten Tanıtmışsın! **12 Saat Sonra Tekrar Tanıtabilirsin!**'
            }});
  }
};
